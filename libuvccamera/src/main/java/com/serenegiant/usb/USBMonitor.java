...
public final class USBMonitor {
	...
	public interface OnDeviceConnectListener {
		...
		/**
		 * called when USB device removed or its power off (this callback is called
		 * after device closing)
		 * 
		 * @param device
		 * @param ctrlBlock
		 */
			...
			for (final UsbDevice device : deviceList.values()) {
				...
				if ((filter == null) || (filter.matches(device) && !filter.isExclude)) {
					...
					if (filter.matches(device)) {
						Log.i("CameraUvc-Devices", "Match Filter");
					}
					if (!filter.isExclude) {
						Log.i("CameraUvc-Devices", "isExclude");
					}
				}
			}
		}
		...
		if (isRegistered()) {
			if (device != null) {
				if (mUsbManager.hasPermission(device)) {
					...
					Log.i("CameraUvc", "connect devices");
					...
					try {
						...
						Log.w(TAG, e);
						Log.i("CameraUvc", "Something wrong when request permission");
						processCancel(device);
						result = true;
					}
				}
			} else {
				Log.i("CameraUvc", "Device is Null");
				processCancel(device);
				result = true;
			}
		} else {
	...
	private final void processConnect(final UsbDevice device) {
		Log.i("CameraUvc", "Process Connect");
		if (destroyed) {
			Log.i("CameraUvc", "Destroyed USB Device");
			return;
		}
		updatePermission(device, true);
		Log.i("CameraUvc", "process connection");
		mAsyncHandler.post(new Runnable() {
			@Override
			public void run() {
				if (DEBUG)
					Log.v(TAG, "processConnect:device=" + device);
				Log.i("CameraUvc", "processConnect:device=" + device);
				UsbControlBlock ctrlBlock;
	 ...
	 * @return
	 */
	private static String getString(final UsbDeviceConnection connection, final int id, final int languageCount,
		...
		for (int i = 1; i <= languageCount; i++) {
			...
			if ((ret > 2) && (work[0] == ret) && (work[1] == USB_DT_STRING)) {
				// skip first two bytes(bLength & bDescriptorType), and copy the rest to the
				// string
				try {
	...
	/**
	 * ベンダー名・製品名・バージョン・シリアルを取得する
	 * #updateDeviceInfo(final UsbManager, final UsbDevice, final
	 * UsbDeviceInfo)のヘルパーメソッド
	 * 
	 * @param context
	 * @param device
	 * @return
	...
	public static UsbDeviceInfo getDeviceInfo(final Context context, final UsbDevice device) {
		return updateDeviceInfo((UsbManager) context.getSystemService(Context.USB_SERVICE), device,
				new UsbDeviceInfo());
	}
	...
	public static final class UsbControlBlock implements Cloneable {
		...
		private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
			...
			if (mConnection != null) {
				...
				Log.i(TAG, String.format(Locale.US, "name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", name, desc, busnum,
						devnum) + rawDesc);
			} else {
		 ...
		 * duplicate by clone
		 * need permission
		 * USBMonitor never handle cloned UsbControlBlock, you should release it after
		 * using it.
		 * 
		 * @return
		 ...
		 * 
		 * @return same value if the devices has same vendor id, product id, device
		 *         class, device subclass and device protocol
		 */
