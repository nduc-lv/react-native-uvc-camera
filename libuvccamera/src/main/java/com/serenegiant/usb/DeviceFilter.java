...
public final class DeviceFilter {
	...
	public DeviceFilter(final int vid, final int pid, final int clasz, final int subclass,
		...
		mVendorId = vid;
		Log.i("CameraUvc-Filter", "Set mVendorId to vid" + vid);
		mProductId = pid;
		mClass = clasz;
		mSubclass = subclass;
		mProtocol = protocol;
	...
	public DeviceFilter(final UsbDevice device, final boolean isExclude) {
		...
		Log.i("CameraUvc-Filter", "set mVendorId to " + device.getVendorId());
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (deviceFilter != null) {
						Log.i("CameraUvc-DeviceFilter", "Add Device Filter from parse context" + deviceFilter.mVendorId);
	 ...
	 * if the value is zero, return false, if the value is non-zero integer, return
	 * true
	 * 
	 * @param context
	 * @param parser
	 ...
	 * @return
	 */
	private static final String getAttributeString(final Context context, final XmlPullParser parser,
	...
	private boolean matches(final int clasz, final int subclass, final int protocol) {
		Log.i("CameraUvc-Filter", mClass + "-" + clasz);
		Log.i("CameraUvc-Filter", mClass + "-" + subclass);
		Log.i("CameraUvc-Filter", mProtocol + "-" + protocol);
	...
	public boolean matches(final UsbDevice device) {
		if (mVendorId != -1 && device.getVendorId() != mVendorId) {
			Log.i("CameraUvc-Filter", "mVendorId" + "-" + device.getVendorId() + "-" + mVendorId);
			return false;
		}
		if (mProductId != -1 && device.getProductId() != mProductId) {
			Log.i("CameraUvc-Filter", "mProductId");
			return false;
		}
		/*
		...
		Log.i("CameraUvc-Filter", "Auto Failed");
		return false;
	}
